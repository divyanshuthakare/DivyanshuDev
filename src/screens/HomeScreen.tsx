...
// ─── Auth flow state type ─────────────────────────────────────────────────────
type FlowState = 'default' | 'phone' | 'otp' | 'email' | 'password';
 ...
 */
function IndiaFlag({ size = 22 }: IndiaFlagProps): React.ReactElement {
  const w      = size * 1.45;
  const h      = size;
...
export default function HomeScreen(): React.ReactElement {
  ...
  // ── Auth state ──────────────────────────────────────────────────────────────
  const [inputValue,   setInputValue]   = useState<string>('');      // shared phone/email field
  const [flowState,    setFlowState]    = useState<FlowState>('default');
  const [otp,          setOtp]          = useState<string[]>(['', '', '', '', '', '']); // 6-digit array
  const [password,     setPassword]     = useState<string>('');
  const [showPassword, setShowPassword] = useState<boolean>(false);   // eye icon toggle
  ...
  const handleInputChange = useCallback((text: string): void => {
    ...
    setFlowState((prev: FlowState): FlowState => {
   ...
   */
  const handleOtpKeyPress = useCallback((key: string, index: number): void => {
   ...
   */
  const handleUsePassword = useCallback((): void => {
    setLoginError(null);
   ...
   */
  const handleOtpContinue = useCallback((): void => {
    const filled = otp.every((d: string) => d !== '');
    if (!filled) return;
    (navigation as any).navigate('InstituteSelection', { identifier: inputValue.trim() });
   ...
   */
  const handlePasswordContinue = useCallback(async (): Promise<void> => {
