...
import { fetchMyInstitutes, selectContext } from '../services/api';
 ...
 *
 * @param {string} text  - Full text to split (e.g. an institute name)
 * @param {string} query - Current search query
 * @returns {{ value: string, match: boolean }[]}
 */
function buildSegments(text, query) {
 ...
 */
function HighlightText({ text, query, isDark, textStyle }) {
 ...
 *
 * Props intentionally passed as primitives/plain values (not the whole institute
 * object for colour tokens) so shallow comparison in memo is reliable.
 */
const InstituteCard = memo((({
...
export default function InstituteSelectionScreen() {
  ...
  const loadInstitutes = useCallback(() => {
    ...
    setError(null);
    fetchMyInstitutes()
      .then(data => { if (!cancelled) setInstitutes(data); })
      .catch(err  => { if (!cancelled) setError(err?.message || 'Failed to load institutes'); })
  ...
  const visibleInstitutes = useMemo(() => {
    ...
    return institutes.filter(i => i.institute_name.toLowerCase().includes(q));
   ...
   */
  const handleSelect = useCallback(async (institute) => {
    ...
    if (roles.length > 1) {
      navigation.navigate('RoleSelection', { institute });
      return;
    }
    ...
    <AppSafeArea style={[styles.safe, { backgroundColor: pageBg }]}>
      ...
      {loading ? (
        ...
        <FlatList
          ...
          keyExtractor={item => String(item.institute_id)}
