...
import { selectContext, Institute, InstituteRole } from '../services/api';
...
const DEFAULT_ROLE_META: RoleMeta = { desc: 'Access your dashboard', color: baseColors.roleGray };
 ...
 */
function getRoleMeta(roleName: string): RoleMeta {
  return (
    ROLE_META[roleName] ||
 ...
 */
function RoleIcon({ roleName, color, size }: RoleIconProps): React.ReactElement {
  const Icon: React.FC<IconProps> = ROLE_ICON_MAP[roleName] || ROLE_ICON_MAP[roleName?.trim()] || DefaultRoleIcon;
...
export default function RoleSelectionScreen(): React.ReactElement {
  ...
  const institute: Institute = route.params.institute;
   ...
   */
  const handleRoleSelect = useCallback(async (role: InstituteRole): Promise<void> => {
    try {
  ...
  const content: React.ReactElement = (
    <>
      ...
      <View style={[styles.roleList, isTablet && styles.roleListTablet]}>
        {(institute.roles || []).map((role: InstituteRole) => {
